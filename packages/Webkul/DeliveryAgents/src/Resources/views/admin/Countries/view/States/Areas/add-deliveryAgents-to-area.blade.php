@if (bouncer()->hasPermission('delivery.deliveryAgent.order'))
    <v-add-delivery-form
    >
    </v-add-delivery-form>
@endif
...
@pushOnce('scripts')
    ...
    >
        @if (bouncer()->hasPermission('delivery.deliveryAgent.range.create'))
            ...
            @if(core()->getConfigData('delivery.settings.ranges.allow_multiple_ranges'))
    <div
        class="primary-button"
        @click="$refs.drawerRef.open()"
    >
        @lang('deliveryAgent::app.country.state.area.view.add-btn')
    </div>
           @endif
        @endif
            ...
            >
                    ...
                    <div class="grid h-8 gap-3">
                        <div class="flex items-center justify-between">
                            <p class="text-xl font-medium dark:text-white">
                                @lang('deliveryAgent::app.country.state.area.view.drawer.header'){{ $Area->area_name }}
                            </p>
                        </div>
                    </div>
                </x-slot:header>
                ...
                <x-slot:content>
                    <div class="box-shadow rounded bg-white p-4 dark:bg-gray-900">
                        <x-admin::datagrid
                            src="{{ route('admin.area.view',['id'=>$Area->id,'mode' => 'out']) }}"
                        >
                            @php
                                $hasPermission = bouncer()->hasPermission('delivery.deliveryAgent.range.create');
                            @endphp
                            <template #header="{
        isLoading,
         available,
    ...
    <script type="module">
        app.component('v-add-delivery-form', {
