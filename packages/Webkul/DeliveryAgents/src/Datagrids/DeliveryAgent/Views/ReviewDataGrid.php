...
class ReviewDataGrid extends DataGrid
{
    ...
    public function prepareColumns()
    {
        $this->addColumn([
            'index'      => 'id',
            'label'      => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.id'),
            'type'       => 'integer',
            'searchable' => true,
            'filterable' => true,
            'sortable'   => true,
        ...
        $this->addColumn([
            ...
            'label'      => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.order_id'),
            'type'       => 'string',
            'searchable' => true,
            'filterable' => true,
            'sortable'   => true,
        ...
        $this->addColumn([
            'index'      => 'customer_name',
            'label'      => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.customer'),
            'type'       => 'string',
            'searchable' => true,
            'filterable' => true,
            'sortable'   => false,
        ...
        $this->addColumn([
            'index'              => 'rating',
            'label'              => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.rating'),
            'type'               => 'string',
            'searchable'         => true,
            'filterable'         => true,
        ...
        $this->addColumn([
            'index'      => 'comment',
            'label'      => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.comment'),
            'type'       => 'string',
            'searchable' => true,
            'filterable' => false,
            'sortable'   => false,
        ...
        $this->addColumn([
            'index'              => 'status',
            'label'              => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.status'),
            'type'               => 'string',
            'searchable'         => true,
            'filterable'         => true,
            'filterable_type'    => 'dropdown',
            'filterable_options' => [
                [
                    'label'  => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.pending'),
                    'value'  => DeliveryAgentReview::STATUS_PENDING,
                ],
                [
                    'label'  => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.approved'),
                    'value'  => DeliveryAgentReview::STATUS_APPROVED,
                ],
                [
                    'label'  => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.disapproved'),
            ...
            'closure'    => function ($row) {
                switch ($row->status) {
                    case DeliveryAgentReview::STATUS_PENDING:
                        return '<p class="label-pending">'.trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.pending').'</p>';

                    case DeliveryAgentReview::STATUS_APPROVED:
                        return '<p class="label-active">'.trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.approved').'</p>';

                    case DeliveryAgentReview::STATUS_DISAPPROVED:
                        return '<p class="label-canceled">'.trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.status.disapproved').'</p>';
        ...
        $this->addColumn([
            'index'           => 'created_at',
            'label'           => trans('deliveryAgent::app.deliveryAgent.reviews.index.datagrid.created_at'),
            'type'            => 'date',
            'searchable'      => true,
            'filterable'      => true,
